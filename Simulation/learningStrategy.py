...
def search_good_strategy(main_env):
    ...
    return [strategies[bs_index], best_strat]
...
def fill_sub_strategy(goal, strat_env):
    ...
    strats_eval = []
    eval_avp = simulate(strats[0], strat_env, 10)
    eval_avp.append(2)
    eval_lm = simulate(strats[1], strat_env, 10)
    eval_lm.append(1)
    eval_rnd = simulate(strats[2], strat_env, 10)
    eval_rnd.append(0)
...
def search_strat(goal, env, strat_name):
    ...
    strat = []
    if strat_name == "avp":
        avps_visit(goal, strat, strat_env, 5)
    elif strat_name == "lm":
        lm_visit(goal, strat, strat_env, 5)
    else:
        rnd_visit(goal, strat, strat_env, 5)
    return strat
